# 03. MACD - トレンドを見える化

## 実践ガイド

**Week 2で詳しく学びます**

MACDを使った取引の実践方法：
- MACDラインとシグナルラインのクロスでトレンド転換を検出
- ヒストグラムで勢いを確認
- 具体的な使い方は、以下の技術解説を読んでから Week 2で実践します

---

## 技術解説 - MACDの誕生

# 📖 Story Chapter 3: MACDの誕生 - トレンドを「見える化」した男

## Scene 1: 1970年代後半、ニューヨーク

**ミコ**: 「次は、RSIよりも少し前の話だ」

**ユウタ**: 「誰？」

**ミコ**: 「ジェラルド・アペル。投資顧問会社を経営してた男だ」

---

### 【回想シーン: アペルの問題意識】

**場所**: ニューヨーク、投資顧問会社のオフィス

**アペル** (投資アナリスト):
「クライアントから毎日同じ質問を受ける...」

**クライアント**: 「今、買い時ですか？売り時ですか？」

**アペル**: 「それに答えるのが俺の仕事だ」

**アペル**: 「でも...どうやって**客観的に**答える？」

*机の上には移動平均線を引いたチャート*

**アペル**: 「移動平均線は使ってる。短期線と長期線のクロスで判断してる」

```markdown
【移動平均線のクロス戦略】

ゴールデンクロス（買いシグナル）:
短期移動平均線が長期移動平均線を下から上に突き抜ける

デッドクロス（売りシグナル）:
短期移動平均線が長期移動平均線を上から下に突き抜ける
```

**アペル**: 「でも、これには問題がある...」

---

## Scene 2: 移動平均線の限界

**アペル**: （チャートを睨む）

```markdown
【移動平均線の3つの問題】

## 問題1: シグナルが遅い
クロスが発生した時には、トレンドがかなり進んでいる
→ 利益の大部分を逃す

## 問題2: ダマシが多い
横ばい相場（レンジ相場）では、何度もクロスが発生
→ 無駄な売買で手数料負け

## 問題3: 「強さ」がわからない
クロスが「強気」なのか「弱気」なのか判断できない
→ 自信を持ってクライアントに勧められない
```

**アペル**: 「移動平均線は『方向』は教えてくれる」

**アペル**: 「でも、『勢い』は教えてくれない」

**アペル**: 「...勢いを可視化できないか？」

---

## Scene 3: 閃き - 「差」に着目

**アペル**: （ペンを走らせる）

```markdown
【思考プロセス】

現状:
短期移動平均線（12日）
長期移動平均線（26日）

観察:
- 上昇トレンド: 短期 > 長期（差が拡大）
- 下降トレンド: 短期 < 長期（差が拡大、マイナス）
- 横ばい: 短期 ≈ 長期（差が縮小）

着想:
2本の線を見るのではなく...
「差」そのものを見ればいいのでは？

定義:
MACD Line = EMA(12) - EMA(26)
（EMA = 指数移動平均）
```

**アペル**: 「これだと、1本の線になる」

**アペル**: 「上昇 = プラス、下降 = マイナス」

**アペル**: 「...でも、まだ足りない」

**アペル**: 「『買い時』『売り時』をもっと明確にしたい」

---

## Scene 4: シグナル線の追加

**アペル**: 「MACD自体も上下する」

**アペル**: 「じゃあ...MACDの移動平均を取ったら？」

```markdown
【第2の着想】

Signal Line = EMA(MACD, 9)
→ MACDの9日間移動平均

判断基準:
- MACD > Signal: 上昇勢い増加 → 買いシグナル
- MACD < Signal: 下降勢い増加 → 売りシグナル
```

**アペル**: 「...これだ！」

**アペル**: （過去のチャートで検証）

**アペル**: 「クロスのタイミングが、移動平均線より早い！」

**アペル**: 「しかも、ダマシが少ない！」

---

## Scene 5: ヒストグラムの発明（後日談）

**ナレーション**:
数年後、トーマス・アスプレイというトレーダーが、MACDをさらに改良した。

**アスプレイ**: 「MACDとSignal Lineの差を、棒グラフにしたらどうだろう？」

```markdown
【ヒストグラムの追加】

Histogram = MACD - Signal

視覚的効果:
- 棒が伸びる: 勢いが強まっている
- 棒が縮む: 勢いが弱まっている
- 棒がゼロをクロス: トレンド転換
```

**ナレーション**:
こうして、現代のMACD（3本セット）が完成した。

---

## Scene 6: 現代、ユウタとミコ

**ユウタ**: 「なるほど...」

**ミコ**: 「アペルは、『勢い』を可視化したかった」

**ミコ**: 「そして、移動平均線の『差』に着目した」

**ユウタ**: 「シンプルだけど、天才的だな」

**ミコ**: 「じゃあ、実装しよう」

---

## Scene 7: MACDの実装

```python
def calculate_macd(prices: pd.Series,
                   fast_period: int = 12,
                   slow_period: int = 26,
                   signal_period: int = 9) -> pd.DataFrame:
    """
    MACD（Moving Average Convergence Divergence）を計算

    Args:
        prices: 終値のSeries
        fast_period: 短期EMA期間（デフォルト12）
        slow_period: 長期EMA期間（デフォルト26）
        signal_period: シグナル線期間（デフォルト9）

    Returns:
        DataFrame（macd, signal, histogram）
    """
    # EMA計算
    ema_fast = prices.ewm(span=fast_period, adjust=False).mean()
    ema_slow = prices.ewm(span=slow_period, adjust=False).mean()

    # MACD Line
    macd = ema_fast - ema_slow

    # Signal Line
    signal = macd.ewm(span=signal_period, adjust=False).mean()

    # Histogram
    histogram = macd - signal

    return pd.DataFrame({
        'macd': macd,
        'signal': signal,
        'histogram': histogram
    })
```

**ユウタ**: 「EMAって何？」

**ミコ**: 「Exponential Moving Average。指数移動平均だ」

```markdown
【EMA vs SMA】

SMA（単純移動平均）:
= (直近N日の合計) / N
→ 全ての日が同じ重み

EMA（指数移動平均）:
= 前日のEMA × (1 - α) + 今日の価格 × α
→ 最近の価格ほど重視

メリット:
SMAより反応が早い
→ トレンド変化を素早く捉える
```

**ユウタ**: 「だからMACDは反応が早いのか」

**ミコ**: 「その通り」

---

## Scene 8: SHIBでの検証

```python
# SHIBの価格データ
shib_df = timeseries_manager.get_data('SHIB')

# MACD計算
macd_df = calculate_macd(shib_df['close'])
shib_df = pd.concat([shib_df, macd_df], axis=1)

# ユウタが買った日
buy_date = '2025-10-18 15:30'

print(f"購入日:")
print(f"  MACD: {shib_df.loc[buy_date, 'macd']:.8f}")
print(f"  Signal: {shib_df.loc[buy_date, 'signal']:.8f}")
print(f"  Histogram: {shib_df.loc[buy_date, 'histogram']:.8f}")
```

**出力**:
```
購入日:
  MACD: 0.00000015
  Signal: 0.00000028
  Histogram: -0.00000013 （マイナス！）
```

**ユウタ**: 「ヒストグラムがマイナス...」

**ミコ**: 「MACD < Signal。つまり、下降の勢いが強まってる」

**ユウタ**: 「だから買った後に下がったのか！」

**ミコ**: 「次に、DOGEを見てみよう」

```python
# DOGEの検証
doge_df = timeseries_manager.get_data('DOGE')
macd_df = calculate_macd(doge_df['close'])
doge_df = pd.concat([doge_df, macd_df], axis=1)

buy_date = '2025-10-18 20:00'
print(f"購入日:")
print(f"  MACD: {doge_df.loc[buy_date, 'macd']:.6f}")
print(f"  Signal: {doge_df.loc[buy_date, 'signal']:.6f}")
print(f"  Histogram: {doge_df.loc[buy_date, 'histogram']:.6f}")
```

**出力**:
```
購入日:
  MACD: 0.002150
  Signal: 0.001890
  Histogram: 0.000260 （プラス！）
```

**ユウタ**: 「ヒストグラムがプラス！」

**ミコ**: 「MACD > Signal。上昇の勢いが強まってる」

**ユウタ**: 「だから勝てたのか！」

---

## Scene 9: MACDの4つの使い方

```markdown
【MACDの4つの使い方】

## 1. ゴールデンクロス/デッドクロス
ゴールデンクロス（買いシグナル）:
MACD が Signal を下から上に突き抜ける
→ Histogram が負→正に転じる

デッドクロス（売りシグナル）:
MACD が Signal を上から下に突き抜ける
→ Histogram が正→負に転じる

## 2. ゼロラインクロス
MACD > 0: 上昇トレンド優勢
MACD < 0: 下降トレンド優勢
ゼロをクロス: トレンド転換の可能性

## 3. ダイバージェンス
価格: 高値更新
MACD: 高値更新せず → 弱気ダイバージェンス

価格: 安値更新
MACD: 安値更新せず → 強気ダイバージェンス

## 4. ヒストグラムの拡大/縮小
ヒストグラム拡大: 勢いが強まっている
ヒストグラム縮小: 勢いが弱まっている（反転注意）
```

**ユウタ**: 「4つもあるのか」

**ミコ**: 「でも、最初は『クロス』だけでいい」

---

## Scene 10: MACDの弱点

**ミコ**: 「MACDにも弱点がある」

```markdown
【MACDの弱点】

## 問題1: レンジ相場でのダマシ
横ばい相場では、何度もクロスが発生
→ 無駄な売買

## 問題2: 遅行指標
移動平均ベースなので、やはり遅れる
→ トレンドの初動は捉えられない

## 問題3: ボラティリティ考慮なし
価格変動の大きさ（ボラティリティ）を考慮しない
→ 急騰/急落には弱い

## 問題4: 絶対値の意味
MACDの値そのものには意味がない
→ 相対的な変化だけを見る
```

**ユウタ**: 「じゃあ、どう使うの？」

**ミコ**: 「RSIと組み合わせる」

---

## Scene 11: RSI × MACD の組み合わせ

**ミコ**: 「2つの指標を組み合わせると、精度が上がる」

```markdown
【RSI × MACD 組み合わせ戦略】

## 強い買いシグナル
✅ RSI < 30（売られすぎ）
✅ MACD ゴールデンクロス（上昇勢い開始）
✅ Histogram が負→正
→ 「底打ち反転」の可能性大

## 強い売りシグナル
✅ RSI > 70（買われすぎ）
✅ MACD デッドクロス（下降勢い開始）
✅ Histogram が正→負
→ 「天井打ち反落」の可能性大

## 弱いシグナル（無視）
❌ RSIとMACDが矛盾
例: RSI < 30 だが MACD デッドクロス
→ 様子見

## フィルタリング
❌ RSI > 70 の時は、MACDが買いでも買わない
❌ RSI < 30 の時は、MACDが売りでも売らない
```

**ユウタ**: 「2つの武器を組み合わせるのか」

**ミコ**: 「そう。単体より、組み合わせの方が強い」

---

## Scene 12: 実装コード（拡張版）

```python
# tools/technical_indicators.py に追加

class TechnicalIndicators:
    # ... (RSI実装は前回) ...

    @staticmethod
    def macd(prices: pd.Series,
             fast: int = 12,
             slow: int = 26,
             signal: int = 9) -> pd.DataFrame:
        """
        MACD（Moving Average Convergence Divergence）

        発明者: Gerald Appel (1970年代後半)
        改良者: Thomas Aspray (ヒストグラム追加)
        用途: トレンドの方向と勢いを判定

        Args:
            prices: 終値
            fast: 短期EMA期間（12）
            slow: 長期EMA期間（26）
            signal: シグナル線期間（9）

        Returns:
            DataFrame（macd, signal, histogram）
        """
        ema_fast = prices.ewm(span=fast, adjust=False).mean()
        ema_slow = prices.ewm(span=slow, adjust=False).mean()

        macd = ema_fast - ema_slow
        signal_line = macd.ewm(span=signal, adjust=False).mean()
        histogram = macd - signal_line

        return pd.DataFrame({
            'macd': macd,
            'signal': signal_line,
            'histogram': histogram
        })

    @staticmethod
    def interpret_macd(macd: float, signal: float, histogram: float,
                       prev_histogram: float = None) -> dict:
        """
        MACD値を解釈

        Args:
            macd: MACD値
            signal: シグナル線
            histogram: ヒストグラム
            prev_histogram: 前日のヒストグラム

        Returns:
            {
                'trend': 'bullish' | 'bearish' | 'neutral',
                'signal': 'buy' | 'sell' | 'hold',
                'strength': 0.0-1.0,
                'description': '説明文'
            }
        """
        # ゴールデンクロス/デッドクロス検出
        if prev_histogram is not None:
            if prev_histogram < 0 and histogram > 0:
                return {
                    'trend': 'bullish',
                    'signal': 'buy',
                    'strength': 0.8,
                    'description': 'ゴールデンクロス検出。上昇トレンド開始の可能性。'
                }
            elif prev_histogram > 0 and histogram < 0:
                return {
                    'trend': 'bearish',
                    'signal': 'sell',
                    'strength': 0.8,
                    'description': 'デッドクロス検出。下降トレンド開始の可能性。'
                }

        # 通常の判定
        if histogram > 0:
            strength = min(abs(histogram) / abs(macd) if macd != 0 else 0, 1.0)
            return {
                'trend': 'bullish',
                'signal': 'hold' if strength < 0.3 else 'buy',
                'strength': strength,
                'description': f'上昇の勢いあり（Histogram={histogram:.6f}）。'
            }
        elif histogram < 0:
            strength = min(abs(histogram) / abs(macd) if macd != 0 else 0, 1.0)
            return {
                'trend': 'bearish',
                'signal': 'hold' if strength < 0.3 else 'sell',
                'strength': strength,
                'description': f'下降の勢いあり（Histogram={histogram:.6f}）。'
            }
        else:
            return {
                'trend': 'neutral',
                'signal': 'hold',
                'strength': 0.0,
                'description': 'トレンドなし。様子見。'
            }

    @staticmethod
    def combined_signal(rsi: float, macd_result: dict) -> dict:
        """
        RSI × MACD 組み合わせシグナル

        Args:
            rsi: RSI値
            macd_result: interpret_macd()の結果

        Returns:
            統合シグナル
        """
        rsi_status = TechnicalIndicators.interpret_rsi(rsi)

        # 強い買いシグナル
        if rsi < 30 and macd_result['signal'] == 'buy':
            return {
                'signal': 'strong_buy',
                'confidence': 0.9,
                'description': '【強い買いシグナル】売られすぎ＋上昇勢い開始。反発の可能性大。'
            }

        # 強い売りシグナル
        if rsi > 70 and macd_result['signal'] == 'sell':
            return {
                'signal': 'strong_sell',
                'confidence': 0.9,
                'description': '【強い売りシグナル】買われすぎ＋下降勢い開始。調整の可能性大。'
            }

        # フィルタリング
        if rsi > 70 and macd_result['signal'] == 'buy':
            return {
                'signal': 'hold',
                'confidence': 0.3,
                'description': '【様子見】買われすぎのため、MACDの買いシグナルは無視。'
            }

        if rsi < 30 and macd_result['signal'] == 'sell':
            return {
                'signal': 'hold',
                'confidence': 0.3,
                'description': '【様子見】売られすぎのため、MACDの売りシグナルは無視。'
            }

        # 通常シグナル
        return {
            'signal': macd_result['signal'],
            'confidence': 0.6,
            'description': f"{rsi_status['description']} {macd_result['description']}"
        }
```

**ユウタ**: 「これで、RSIとMACDの組み合わせができた！」

**ミコ**: 「そう。2つの武器が揃った」

---

## Scene 13: エピローグ

**ユウタ**: 「RSIは『過熱』、MACDは『勢い』」

**ミコ**: 「よく理解できてるな」

**ユウタ**: 「でも、まだ足りない気がする」

**ミコ**: 「鋭いな。確かに、まだ足りない」

**ミコ**: 「次は...『リスク』を測る武器だ」

---

## 📝 Chapter 3 まとめ

### MACD（Moving Average Convergence Divergence）

```markdown
【発明】
- 発明者: Gerald Appel (1970年代後半)
- 改良者: Thomas Aspray (ヒストグラム追加)
- 動機: 移動平均線のクロスを改良し、「勢い」を可視化

【計算式】
MACD Line = EMA(12) - EMA(26)
Signal Line = EMA(MACD, 9)
Histogram = MACD - Signal

【解釈】
- Histogram > 0: 上昇の勢い
- Histogram < 0: 下降の勢い
- ゴールデンクロス: 買いシグナル
- デッドクロス: 売りシグナル

【RSI × MACD 組み合わせ】
強い買い: RSI < 30 ＋ MACD ゴールデンクロス
強い売り: RSI > 70 ＋ MACD デッドクロス
```

### 実装完了

✅ `tools/technical_indicators.py` 拡張
- `macd()`: MACD計算
- `interpret_macd()`: MACD解釈
- `combined_signal()`: RSI×MACD統合シグナル

---

## 🛠️ 実践で使う

MACDを実際の取引で活用する方法：
- **Week 2: テクニカル分析の実践**（作成予定） - MACDでトレンド方向を判断
  - ゴールデンクロス（買いシグナル）
  - デッドクロス（売りシグナル）
  - Histogramでモメンタム確認

---

**Next Chapter**: [Chapter 4: Bollinger Bandsの発明 - 「リスク」を可視化した男](./04_bollinger_bands_invention.md)

---

## 📖 次へ

次のファイル: **[04. Bollinger Bands - リスクを可視化](./04_bollinger_bands.md)**

